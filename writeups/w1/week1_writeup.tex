...
However, I believe they misclassified its role in the emergence of a resist subgroup.

Namely, the student report suggested a modified gossip algorithm such that an unhappy subgroup
has a separate gossip network on top of the collective network for the entire group.
I believe this should be a byproduct of the existence of an unhappy and should form on its own.
...
which can be verified empirically for sufficiently large $t$ close to norm convergence. This drives
...
\[ \operatorname{sim}_t(i, j, s) = \begin{cases}
    \operatorname{sim}_{t - 1}(i, j, s) \cdot (1 - \beta) + \beta \Big(u_i(t, s, \pi_i(s)) - u_j(t, s, \pi_j(s))\Big)^2 & t > 0 \\
    0 & t = 0
\end{cases}
\]
for a fixed rate constant $\beta \in (0, 1)$
then
\[ \operatorname{sim}_t(i, j) = \frac{1}{|S|}\sum_{s \in S}\operatorname{sim}^t(i, j, s) \]
then there is a fixed constant $C$ such that agents $i$ and $j$ could gossip at time $t$
only if
\[ \operatorname{sim}(i, j) < C \]
and it is clear that unhappy agents would have a low score with each other
...
is unchanged when the agent is optimizing status or reputation. Additionally, for an agent that is has adopted the dominant
schema as a follower of the current leader, it only acts under the policy of the leader with out the chance to sample from its own
personal utility weights where its personal utility estimate could rise above its reputation estimate and it would detach from the group.
...
\[ \{ \mu_i(s)\}_{s\in S} = \operatorname{argmax}_{\mu_i} \sum_{s \in S} \theta_\alpha(\mu_{\text{min}(s)} + \mu_i(s))\cdot u_i(s, \pi_i(s)) \]
subject to the constraints
\[ \sum_{s \in S}\mu_{\text{min}}(s) + \mu_i(s) \leq M  \quad 0 \leq \mu_s \]
where the agent i's interaction rate becomes $\mu_{\text{total}} = \mu_{\text{min}} + \mu_i$
here $M$ is a budget on the total attention an actor $i$ can allocate to different situations and
...
\[ \mu_i(s)^* = 0 \]
then for $s \in F$
\[ \mu_i(s)^* = \frac{M}{|F|} + \Big(\mu_{\text{min}}(s) - \frac{1}{|F|}\sum_{t \in P}\mu_{min}(t)\Big) + \Big(\log u_i(s, \pi_i(s)) - \frac{1}{|F|}\sum_{t \in F}\log u_i(t, \pi_i(t))\Big) \]
...
that it replaces the current model for $\mu_{i, p}$.

Intuitively a hypothesis would be that as the system progresses and the resistant subgroup
for a given member $s \in G_r$ its free states will correspond roughly to $S_r$
...
For each agent $i$, given some initialization of $\mu_{i, }(s, 0)$ for all $s \in S$ that satisfies
\[ \sum_{s \in S}\mu_{i, p}(s) + \mu_{\text{min}}(s) \leq M \quad \forall s \in S, \mu_{i, p}(s) \geq 0  \]
at time $t$ the above optimization problem is solved and $\mu_{i, p}(s, t)^*$ are found for $s \in S$.
Then there is a fixed rate constant $\eta \in (0, 1)$ such that the rate is updated to
\[ \mu_{i, p}(s, t+1) = (1 - \eta)\mu_{i, p}(s, t) + \eta \cdot \mu_{i, p}(s, t)^* \]
and this clearly satisfies the constraints as $\{ \mu_{i, p}(s, t) \}_{s \in S}$ and $\{ \mu_{i, p}(s, t)^* \}_{s \in S}$
both do.

Now to actually calculate $\mu_i(s)^*$ in practice $F, P$ and have to be computed.
...
Hence we can define
\[ L_k = \sum_{j \leq k}u_i(s_j, \pi_i(s_j))\mu_{\text{min}}(s_j) + \sum_{j > k}u_i(s_j, \pi_i(s_j))\mu_{i, p}(s_j, t) \]
...
from a static participant interaction rate to this attention-based one has to be done carefully.
...
\[ u_i(s, \sigma_r) \sim \mathcal N(\mu_r^{G_r}, {\sigma_r^{G_r}}^2) \quad \forall i \in G_r\]
and
\[ u_i(s, \sigma_r) \sim \mathcal N(\mu_r^{G}, {\sigma_r^{G}}^2) \quad \forall i \in G \setminus G_r\]
then for the dominant schema $\sigma d$:
\[ u_i(s, \sigma_d) \sim \mathcal N(\mu_d^{G_r}, {\sigma_d^{G_r}}^2) \quad \forall i \in G_r \]
and
\[ u_i(s, \sigma_d) \sim \mathcal N(\mu_d^{G}, {\sigma_d^{G}}^2) \quad \forall i \in G \setminus G_r\]
...
would have been necessary for any behaviour to emerge. However, my proposed changes to personal utility tracking and follower deviation,
...
this is not optimal as it assumes the effect of each parameter is additive. Instead of this,
since I have already cut down the number of parameters in the simulation I propose running a grid of different parameter values.
